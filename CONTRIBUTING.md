...
1. Install [mise](https://mise.jdx.dev/getting-started.html), then run the following command to fetch the
   pinned tools from the repo root (hk, rumdl, zizmor — see
...
3. Install the git hooks — recommended once per machine (Git 2.54+, silent
...
- `hk check --all` verifies everything (format, lints, analyzer, markdown, workflows). It doesn't modify files. `hk fix --all` on the other hand applies what it can — formatting, fixes, and
  codegen.
